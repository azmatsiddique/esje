"""PostgreSQL driver implementation for esje using psycopg2/psycopg and SQLAlchemy."""
...
class PostgresDriver(BaseDriver):
    """PostgreSQL driver utilizing SQLAlchemy with psycopg2 or psycopg."""
    ...
    def connect(self) -> None:
        if self._engine is None:
            ...
            try:
                ...
                raise ConnectionError(
                    f"PostgreSQL connection engine creation failed: {exc}",
        ...
        try:
            ...
            if "password authentication failed" in msg_lower or "fatal: password" in msg_lower:
                hint = "Access denied: verify your PostgreSQL username and password."
            elif "could not connect to server" in msg_lower or "connection refused" in msg_lower:
                hint = f"Failed to connect to PostgreSQL server at {self.host}:{self.port}. Ensure the server is running."
            elif "database" in msg_lower and "does not exist" in msg_lower:
                hint = f"Database '{self.database}' does not exist on target host."
            raise ConnectionError(f"PostgreSQL connection failed: {msg}", hint=hint) from (
                exc if config.verbose_errors else None
            )
        except Exception as exc:
            if isinstance(exc, ConnectionError):
                raise
            raise ConnectionError(f"PostgreSQL connection failed: {exc}") from (
                exc if config.verbose_errors else None
            )
